<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="任务编号" prop="renwubianhao">
          <el-select  @change="renwubianhaoChange" v-model="ruleForm.renwubianhao" placeholder="请选择任务编号">
            <el-option
                v-for="(item,index) in renwubianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/renwu/renwubianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.renwubianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    renwubianhaoChange () {
      this.$http({
        url: `follow/renwu/renwubianhao?columnValue=`+ this.ruleForm.renwubianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.jifen){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zengjifen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zengjifenCrossAddOrUpdateFlag = false;
                  this.parent.search();

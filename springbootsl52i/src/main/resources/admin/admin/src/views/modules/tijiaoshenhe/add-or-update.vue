<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="任务积分" prop="renwujifen">
          <el-input v-model="ruleForm.renwujifen" 
              placeholder="任务积分" clearable  :readonly="ro.renwujifen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="任务积分" prop="renwujifen">
              <el-input v-model="ruleForm.renwujifen" 
                placeholder="任务积分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="提交审核文件" prop="tijiaoshenhewenjian">
          <file-upload
          tip="点击上传提交审核文件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.tijiaoshenhewenjian?ruleForm.tijiaoshenhewenjian:''"
          @change="tijiaoshenhewenjianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.tijiaoshenhewenjian" label="提交审核文件" prop="tijiaoshenhewenjian">
            <el-button type="text" size="small" @click="download(ruleForm.tijiaoshenhewenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tijiaoshenhe/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tijiaoshenheCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    tijiaoshenhewenjianUploadChange(fileUrls) {
	this.ruleForm.tijiaoshenhewenjian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{

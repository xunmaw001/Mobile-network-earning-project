<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="物品编号" prop="wupinbianhao">
          <el-select  @change="wupinbianhaoChange" v-model="ruleForm.wupinbianhao" placeholder="请选择物品编号">
            <el-option
                v-for="(item,index) in wupinbianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/duihuanwupin/wupinbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.wupinbianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    wupinbianhaoChange () {
      this.$http({
        url: `follow/duihuanwupin/wupinbianhao?columnValue=`+ this.ruleForm.wupinbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.jifen){
                this.ruleForm.jifen = data.data.jifen
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiaojifen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiaojifenCrossAddOrUpdateFlag = false;
                  this.parent.search();

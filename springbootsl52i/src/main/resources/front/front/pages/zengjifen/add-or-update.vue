<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(21, 20, 20, 1)","textAlign":"left"}' class="title">任务编号</view>
				<picker @change="renwubianhaoChange" :value="renwubianhaoIndex" :range="renwubianhaoOptions">
					<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(20, 19, 19, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{renwubianhaoOptions[renwubianhaoIndex]}}</view>
				</picker>
			</view>
		...
		async onLoad(options) {
			...
			this.user = res.data;
						...
						// ss读取
																														...
																														// 下2
			res = await this.$api.option(`renwu`,`renwubianhao`,{});
			this.renwubianhaoOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zengjifen`, this.ruleForm.id);
		...
		methods: {
			styleChange() {
				this.$nextTick(()=>{
					...
					// })
																														...
																														// 下二随
			async renwubianhaoChange (e) {
				this.renwubianhaoIndex = e.target.value
				this.ruleForm.renwubianhao = this.renwubianhaoOptions[this.renwubianhaoIndex]
				let res = await this.$api.follow(`renwu`, `renwubianhao`,{
					columnValue: this.ruleForm.renwubianhao
				});
																												if(res.data.jifen){
			...
			async onSubmitTap() {
				...
				await this.$api.update(`${table}`, obj);
																																																																																																																																																																								if(this.ruleForm.id){
					await this.$api.update(`zengjifen`, this.ruleForm);
				}else{
					await this.$api.add(`zengjifen`, this.ruleForm);
				}

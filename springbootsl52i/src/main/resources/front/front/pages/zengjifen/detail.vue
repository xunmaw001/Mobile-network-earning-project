...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zengjifen');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','zengjifen');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('zengjifen', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusszengjifen/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('zengjifen', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('zengjifen', this.detail);
				this.$utils.msg('审核成功');
